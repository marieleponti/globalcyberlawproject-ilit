...
def uof_sov_parallel_categories(request):
    ...
    fig.add_trace(go.Parcats(
        dimensions=[
            ...
            {
                'label': questions_uof[0],
                'values': df_uof[questions_uof[0]]
            },
            {
                'label': questions_sov[1],
                'values': df_sov[questions_sov[1]]}
        ],
        line={'color': df_uof.index, 'colorscale': 'Viridis'},
        arrangement='freeform',
        # Configuración para ocultar etiquetas en nodos y mostrarlas en leyenda
        # Añadir información para la leyenda
